...
//            }
            changeDirectionIfNecessary(*e);
        } catch (std::bad_cast&) {
            try {
                foodCollisioncheck1(*e);
...
void Controller::changeDirection(char dir){
...
void Controller::lostCheck1(bool& lost, const Segment& newHead){
...
void Controller::lostCheck2(bool& lost, const Segment& newHead){
...
void Controller::lostCheck3(const Segment& newHead){
...
void Controller::lostCheck(bool& lost, const Segment& newHead){
    lostCheck1(lost, newHead);
//        for (auto segment : m_segments) {
//            if (segment.x == newHead.x and segment.y == newHead.y) {
//                m_scorePort.send(std::make_unique<EventT<LooseInd>>());
//                lost = true;
...
void Controller::changeDirectionIfNecessary(const Event& e){
    auto direction = dynamic_cast<EventT<DirectionInd> const&>(e)->direction;
...
void Controller::foodCollisioncheck1(const Event& e){
    auto receivedFood = *dynamic_cast<EventT<FoodInd> const&>(e);

    bool requestedFoodCollidedWithSnake = false;

    void foodCollisioncheck1(bool& foodCollision);
    for (auto const& segment : m_segments) {
        if (segment.x == receivedFood.x and segment.y == receivedFood.y) {
            requestedFoodCollidedWithSnake = true;
            break;
...
void Controller::foodCollisioncheck2(const Event& e){
    auto requestedFood = *dynamic_cast<EventT<FoodResp> const&>(e);
