...
bool Controller::checkIfSnakeRunIntoOwnSegment(Segment newHead){
...
void Controller::drawNewMovedSnake(bool lost,Segment newHead){
    if (not lost) {
...
Snake::Controller::Segment Controller::setNewHead(){
...
void Controller::moveSnake(){
    ...
    bool lost = false;
    lost = checkIfSnakeRunIntoOwnSegment(newHead);
    ...
    drawNewMovedSnake(lost, newHead);    
}

void Controller::setDirection(Snake::Direction direction){
    if ((m_currentDirection & 0b01) != (direction & 0b01)) {
        m_currentDirection = direction;
    }
}

void Controller::manageReceivedFood(Snake::FoodInd receivedFood){
...
void Controller::manageRequestedFood(Snake::FoodResp requestedFood){
    bool requestedFoodCollidedWithSnake = false;
    for (auto const& segment : m_segments) {
    ...
    try {
        ...
        try {
            ...
            try {
                ...
                manageReceivedFood(receivedFood);
            } catch (std::bad_cast&) {
                try {
                    auto requestedFood = *dynamic_cast<EventT<FoodResp> const&>(*e);
                    manageRequestedFood(requestedFood);
